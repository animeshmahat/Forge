...
<div class="d-flex flex-column">
    <h2 class="align-top">{{$_panel}} Settings</h2>
    <div class="row">
        <!-- information -->
        <div class="col-6 card shadow mb-auto mx-3 p-4">
            <h4><i class="fa fa-user-circle" aria-hidden="true"></i>&nbsp;Edit {{$_panel}} Information</h4>
            <hr>
            <form action="{{route('blogger.profile.update', ['id' => $data['row']->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                ...
                <hr>
                <div>
                    <a href="{{ route('blogger.index') }}" class="btn btn-success btn-sm"><i class="fa fa-ban"></i> CANCEL</a>
        ...
        <div class=" col-5 card shadow mb-auto mx-3 p-4">
            ...
            <hr>
            <form action="{{ route('blogger.profile.passwordChange', ['id' => $data['row']->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <div class="form-group mb-3">

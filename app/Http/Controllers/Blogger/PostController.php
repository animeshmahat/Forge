...
{
    protected $base_route = "blogger.post.index";
    protected $view_path = "blogger.post";
    ...
    {
        $data['row'] = Posts::where('user_id', auth()->id())->with(['user', 'category'])->get();
    ...
    {
        $data['row'] = Posts::where('user_id', auth()->id())->with(['user', 'category', 'tags'])->findOrFail($id);
    ...
    protected function createFolderIfNotExist($folderPath)
    {
